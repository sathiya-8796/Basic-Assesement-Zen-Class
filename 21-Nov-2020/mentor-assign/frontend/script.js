function makeMentorRows(data, status) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        th0.innerHTML =
            "<input type='radio' name='mentors_radio' value='" +
            data[i].mentor_id +
            "'>";
        ...
        th3.innerHTML =
            '<a href="#" class="badge badge-info" onclick="getStudentsList(' +
            data[i].mentor_id +
            ')">Show students</a>';
...
let makeStudentRows = function (data, status) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (status === 1) {
            th0.innerHTML =
                "<input type='checkbox' name='students_box' value='" +
                data[i].student_id +
                "'>";
        } else if (status == 2) {
            th0.innerHTML =
                "<input type='radio' name='students_radio' value='" +
                data[i].student_id +
                "'>";
        }
        let th1 = document.createElement("th");
...
let action = 0;
let url = "https://assign-mentor-db.herokuapp.com";
...
let assignMentor = () => {
    ...
    document.getElementById("alert-msg").innerHTML =
        "<strong>NOTE :</strong> You can select one student and can change or assign a mentor...";
};
...
let assignStudents = () => {
    ...
    document.getElementById("alert-msg").innerHTML =
        "<strong>NOTE :</strong> You can select multiple idle students and can assign to one mentor...";
};
...
let assignButton = () => {
    if (action == 1) {
        let mt_id = parseInt(
            document.querySelector('input[name="mentors_radio"]:checked').value
        );
        let checkboxes = document.getElementsByName("students_box");
        let valids = [];
        for (var checkbox of checkboxes) {
            if (checkbox.checked) valids.push(+checkbox.value);
        }
        ...
        let st_id = parseInt(
            document.querySelector('input[name="students_radio"]:checked').value
        );
        let mt_id = parseInt(
            document.querySelector('input[name="mentors_radio"]:checked').value
        );
